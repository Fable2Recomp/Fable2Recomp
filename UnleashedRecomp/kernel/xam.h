...
bool XNotifyGetNext(uint32_t hNotification, uint32_t dwMsgFilter, xe::be<uint32_t>* pdwId, xe::be<uint32_t>* pParam);

uint32_t XamShowMessageBoxUI(uint32_t dwUserIndex, xe::be<uint16_t>* wszTitle, xe::be<uint16_t>* wszText, uint32_t cButtons,
    xpointer<xe::be<uint16_t>>* pwszButtons, uint32_t dwFocusButton, uint32_t dwFlags, xe::be<uint32_t>* pResult, XXOVERLAPPED* pOverlapped);

uint32_t XamContentCreateEnumerator(uint32_t dwUserIndex, uint32_t DeviceID, uint32_t dwContentType,
    uint32_t dwContentFlags, uint32_t cItem, xe::be<uint32_t>* pcbBuffer, xe::be<uint32_t>* phEnum);
uint32_t XamEnumerate(uint32_t hEnum, uint32_t dwFlags, void* pvBuffer, uint32_t cbBuffer, xe::be<uint32_t>* pcItemsReturned, XXOVERLAPPED* pOverlapped);

uint32_t XamContentCreateEx(uint32_t dwUserIndex, const char* szRootName, const XCONTENT_DATA* pContentData,
    uint32_t dwContentFlags, xe::be<uint32_t>* pdwDisposition, xe::be<uint32_t>* pdwLicenseMask,

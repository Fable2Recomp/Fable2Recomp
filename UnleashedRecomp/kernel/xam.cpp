...
namespace XAM
{
    uint32_t GetState(uint32_t userIndex, XAMINPUT_STATE* state)
    {
        ...
        return hid::GetState(userIndex, state);
    }
    ...
    uint32_t GetCapabilities(uint32_t userIndex, XAMINPUT_CAPABILITIES* capabilities)
    {
        ...
        return hid::GetCapabilities(userIndex, capabilities);
    }

    void SetProhibitedInputs(uint32_t prohibitedButtons, bool isLeftStickProhibited, bool isRightStickProhibited)
    {
        hid::SetProhibitedInputs(prohibitedButtons, isLeftStickProhibited, isRightStickProhibited);
    }

    bool IsInputAllowed()
    {
        return hid::IsInputAllowed();
    }

    bool IsInputDeviceController(uint32_t userIndex)
    {
        return hid::IsInputDeviceController(userIndex);
    }

    const char* GetInputDeviceName(uint32_t userIndex)
    {
        return hid::GetInputDeviceName(userIndex);
    }
}

// Guest function hooks
GUEST_FUNCTION_HOOK(sub_82BD7788, XAM::GetState);
GUEST_FUNCTION_HOOK(sub_82BD8600, XAM::SetState);
GUEST_FUNCTION_HOOK(sub_82BD88F0, XAM::GetCapabilities);
GUEST_FUNCTION_HOOK(sub_82BD6FD0, XAM::SetProhibitedInputs);
GUEST_FUNCTION_HOOK(sub_831CC9C8, XAM::IsInputAllowed);
GUEST_FUNCTION_HOOK(sub_831CCA60, XAM::IsInputDeviceController);
GUEST_FUNCTION_HOOK(sub_831CCB00, XAM::GetInputDeviceName);

static int StringHash(std::string_view key) {
	int hash = 0;
	for (int i = 0; key[i]; i++) {
		hash = hash * 31 + key[i];
	}

...
#define CONFIG_DEFINE_LOCALISED(section, type, name, defaultValue) \
    ConfigDef<type> Config::name(section, #name, nullptr, defaultValue);

#undef  CONFIG_DEFINE_ENUM
#define CONFIG_DEFINE_ENUM(section, type, name, defaultValue) \
    ConfigDef<type> Config::name(section, #name, defaultValue, nullptr);

#undef  CONFIG_DEFINE_ENUM_LOCALISED
#define CONFIG_DEFINE_ENUM_LOCALISED(section, type, name, defaultValue) \
    ConfigDef<type> Config::name(section, #name, nullptr, defaultValue, nullptr, nullptr);

#include "config_def.h"

// CONFIG_DEFINE
template<typename T, bool isHidden>
ConfigDef<T, isHidden>::ConfigDef(std::string section, std::string name, T defaultValue)
    : Section(std::move(section))
    , Name(std::move(name))
    , DefaultValue(defaultValue)
    , Value(defaultValue)
{
    g_configDefinitions.push_back(this);
}

// CONFIG_DEFINE_LOCALISED
template<typename T, bool isHidden>
ConfigDef<T, isHidden>::ConfigDef(std::string section, std::string name, CONFIG_LOCALE* nameLocale, T defaultValue)
    : Section(std::move(section))
    , Name(std::move(name))
    , Locale(nameLocale)
    , DefaultValue(defaultValue)
...
// CONFIG_DEFINE_ENUM
template<typename T, bool isHidden>
ConfigDef<T, isHidden>::ConfigDef(std::string section, std::string name, T defaultValue, std::unordered_map<std::string, T>* enumTemplate)
    : Section(std::move(section))
    , Name(std::move(name))
    , DefaultValue(defaultValue)
    , Value(defaultValue)
...
{
    if (EnumTemplate) {
        for (const auto& [key, value] : *EnumTemplate) {
            EnumTemplateReverse[value] = key;
        }
    }
    g_configDefinitions.push_back(this);
}
...
template<typename T, bool isHidden>
ConfigDef<T, isHidden>::ConfigDef(std::string section, std::string name, CONFIG_LOCALE* nameLocale, T defaultValue, std::unordered_map<std::string, T>* enumTemplate, CONFIG_ENUM_LOCALE(T)* enumLocale)
    : Section(std::move(section))
    , Name(std::move(name))
    , Locale(nameLocale)
    , DefaultValue(defaultValue)
...
template<typename T, bool isHidden>
ConfigDef<T, isHidden>::~ConfigDef() {
    auto it = std::find(g_configDefinitions.begin(), g_configDefinitions.end(), this);
    if (it != g_configDefinitions.end())
        g_configDefinitions.erase(it);
}
...
void ConfigDef<T, isHidden>::ReadValue(toml::v3::ex::parse_result& toml) {
    try {
        if (auto val = toml[Section][Name].value<T>()) {
            Value = *val;
...
std::string ConfigDef<T, isHidden>::GetValueLocalised(ELanguage language) const {
    if (EnumLocale) {
        auto it = EnumLocale->find(language);
        if (it != EnumLocale->end()) {
            auto valueIt = it->second.find(Value);
            if (valueIt != it->second.end())
                return std::get<0>(valueIt->second);
        }
    }
    if (EnumTemplate) {
...
std::string ConfigDef<T, isHidden>::GetValueDescription(ELanguage language) const {
    if (EnumLocale) {
        ...
        if (it != EnumLocale->end()) {
            auto valueIt = it->second.find(Value);
            if (valueIt != it->second.end())
                return std::get<1>(valueIt->second);
        }
    }
    return "";
}
...
void ConfigDef<T, isHidden>::GetLocaleStrings(std::vector<std::string_view>& localeStrings) const {
    if (Locale) {
        for (const auto& [language, strings] : *Locale) {
            localeStrings.push_back(std::get<0>(strings));
            localeStrings.push_back(std::get<1>(strings));
        }
    }
    if (EnumLocale) {
        for (const auto& [language, values] : *EnumLocale) {

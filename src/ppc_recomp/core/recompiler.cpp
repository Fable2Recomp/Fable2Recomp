...
void Recompiler::RecompileBranch(const DecodedInstruction& inst, std::stringstream& ss, Block& block) {
    switch (inst.opcode) {
        case Opcode::B:
        ...
        case Opcode::BCLR:
            ss << GetConditionCode(inst) << " " << "lr\n";
            block.is_terminated = true;
            break;
            
        case Opcode::BCCTR:
            ss << GetConditionCode(inst) << " " << "ctr\n";
            block.is_terminated = true;
            break;
    }
}
